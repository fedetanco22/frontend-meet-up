...
const User = () => {
    ...
    const getRoles = async () => {
        if (user?.data?.role === 'Administrator') {
            const url = `${process.env.APP_REACT_MEET_UP}/roles`;
            try {
    ...
    const confirmChange = async (e) => {
        setIsLoading(true);
        e.preventDefault();
        const url = `${process.env.APP_REACT_MEET_UP}/users/changeRole/` + editUser?.data?.user_id;
        try {
    ...
    const getEditUser = async () => {
        setIsLoading(true);
        const url = `${process.env.APP_REACT_MEET_UP}/user/` + id;
        try {
            const res = await axios.get(`${url}`, {
                headers: { Authorization: `Bearer ${user.token}` },
            });
