...
console.log(image, 'image en add')
  ...
  const saveImage = async () => {
    ...
    try {
      const res = await axios.post(url, formData, {headers: {"Content-Type": "multipart/form-data", Authorization: `Bearer ${user.token}`}});
      if (res.status === 200) {
        setSend(true);
        setSendError(false);
        setIsPending(false);
  ...
  const fotoTmp =
    imageUrl !== null ? <Image src={imageUrl} alt={"course"} layout="fill" objectFit="cover" objectPosition="50% 50%" quality={100} /> : null;
  const foto =
    image !== undefined && image !== null  ? (
      <Image
        src={`http://164.92.76.51:3000/coursesImages/${image}`}
        alt={"course"}
        layout="fill"
        objectFit="cover"
      ...
      <div className="d-flex flex-wrap">
        <div className="col-12 col-md p-4 my-3">
          ...
          <div className="col-12 col-md-auto text-end px-md-3">
            <Button text={t("form.submit")} className="w-100" buttonType={"light"} disabled={!isPending} callback={() => saveImage()} asSubmit>
              <FaSave />
            </Button>
          </div>
        </div>
